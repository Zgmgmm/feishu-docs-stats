...
# 导入授权相关函数
from auth_utils import get_user_access_token
...
class DocumentStats:
    ...
    def from_api_stats(cls, node, stats, source_url, is_root=False):
        ...
        token = (
            node.obj_token
            if hasattr(node, "obj_token") and node.obj_token
            else node.node_token
        )
        if is_root:
            node_url = source_url
        else:
...
def parse_lark_url(url: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        if file_type.lower() in ["wiki", "docx"]:
            ...
            logger.warning(
                f"URL中的文件类型 '{file_type}' 未知或不支持直接统计，将作为原始类型传递"
            )
...
def get_node_space_id(node_token: str) -> Optional[str]:
    ...
    Returns:
        空间ID，获取失败则返回None
    """
    request = GetNodeSpaceRequest.builder().token(node_token).obj_type("wiki").build()
    ...
    try:
        ...
        if not response.success():
            logger.error(
                f"获取空间ID失败 (token: {node_token}): {response.code} {response.msg}"
            )
            return None
...
def get_all_child_nodes(space_id: str, parent_node_token: str = None) -> List[Any]:
    ...
    while True:
        ...
        try:
            response: ListSpaceNodeResponse = client.wiki.v2.space_node.list(
                request, options
            )
            if not response.success():
                logger.error(
                    f"扫描Wiki文档失败 (space_id: {space_id}, parent: {parent_node_token}): {response.code} {response.msg}"
                )
                break
            ...
            logger.exception(
                f"扫描Wiki文档异常 (space_id: {space_id}, parent: {parent_node_token}): {e}"
            )
            break
...
async def get_all_child_nodes_async(
    space_id: str, user_token: str, parent_node_token: str = None
) -> List[Any]:
    """异步递归获取所有子节点
    ...
    while True:
        ...
        try:
            ...
            response: ListSpaceNodeResponse = await loop.run_in_executor(
                None, lambda: client.wiki.v2.space_node.list(request, options)
            )
            ...
            if response.data and response.data.items:
                for item in response.data.items:
                    ...
                    # 递归获取子节点（异步）
                    if item.has_child:
                        child_nodes = await get_all_child_nodes_async(
                            space_id, user_token, item.node_token
                        )
                        all_nodes.extend(child_nodes)
...
async def get_node_space_id_async(node_token: str, user_token: str) -> Optional[str]:
    ...
    try:
        ...
        response: GetNodeSpaceResponse = await loop.run_in_executor(
            None, lambda: client.wiki.v2.space.get_node(request, options)
        )
        if not response.success():
            logger.error(
                f"获取空间ID失败 (token: {node_token}): {response.code} {response.msg}"
...
def batch_get_meta(docs: List[RequestDoc], user_token: str = None) -> List[Meta]:
    ...
    for i in range(0, len(docs), BATCH_SIZE):
        ...
        logger.info(
            f"处理第 {batch_num}/{total_batches} 批，包含 {len(batch_docs)} 个文档"
        )
...
def batch_get_meta_single(docs: List[RequestDoc], user_token: str = None) -> List[Meta]:
    ...
    request: BatchQueryMetaRequest = (
        ...
        .request_body(MetaRequest.builder().request_docs(docs).with_url(False).build())
        .build()
    )

    try:
        options = lark.RequestOption.builder().user_access_token(user_token).build()
        response: BatchQueryMetaResponse = client.drive.v1.meta.batch_query(
            request, options
        )
...
def batch_get_stats(
    file_tokens: List[Tuple[str, str]], user_token: str = None
) -> Dict[Tuple[str, str], Any]:
    """同步批量获取文档统计信息
...
def get_doc_update_time(file_token: str, file_type: str, user_token: str = None) -> int:
    ...
    try:
        ...
        logger.exception(
            f"获取文档更新时间失败 (file_token: {file_token}, type: {file_type}): {e}"
        )
        return 0


def collect_node_stats(
    node: Union[NodeInfo, Any],
    stats: Optional[Any],
    source_url: str,
    user_token: str = None,
    is_root: bool = False,
...
def build_wiki_tree(space_id: str, root_node_token: str) -> Optional[WikiNode]:
    ...
    try:
        ...
        tree = WikiNode(
            title=root_node.title, token=root_node.node_token, type=root_node.obj_type
        )
...
def _get_root_node_info(root_node_token: str) -> Optional[Any]:
    ...
    """
    node_info_request = (
        GetNodeSpaceRequest.builder().token(root_node_token).obj_type("wiki").build()
    )
    user_token = get_user_access_token()
    if not user_token:
        logger.error("未找到有效的user_access_token")
    ...
    try:
        node_info_resp = client.wiki.v2.space.get_node(
...
def _build_subtree(space_id: str, parent_token: str, current_tree: WikiNode) -> None:
    ...
    for node in child_nodes:
        child = WikiNode(title=node.title, token=node.node_token, type=node.obj_type)
...
def print_wiki_tree(
    tree: Optional[WikiNode], indent: int = 0, prefix: str = ""
) -> None:
    """打印Wiki树形结构
...
def export_wiki_tree(
    tree: Optional[WikiNode], filename: str = "wiki_tree.json"
) -> None:
    """将Wiki树结构导出为JSON文件
...
def process_wiki_node(
    file_token: str, space_id: str, url: str
) -> Tuple[Optional[Dict[str, Any]], Optional[WikiInfo]]:
    """处理Wiki节点
    ...
    try:
        ...
        # 获取文档标题和更新时间
        request_doc = (
            RequestDoc.builder().doc_token(file_token).doc_type("wiki").build()
        )
        ...
        # 创建NodeInfo对象，使用真实标题
        node_info = NodeInfo(
            title=title, node_token=file_token, obj_token=file_token, obj_type="wiki"
        )
        ...
        # 创建WikiInfo对象
        wiki_info = WikiInfo(token=file_token, space_id=space_id, url=url)
...
def process_wiki_children(
    space_id: str, parent_token: str, url: str
) -> List[Dict[str, Any]]:
    """处理Wiki的所有子节点（优化版本）
        ...
        for node in child_nodes:
            if node.obj_token and node.obj_type:
                ...
                if node.obj_token in meta_dict:
                    ...
                    logger.debug(
                        f"节点 {node.obj_token} 未找到元数据，使用原始标题: {title}"
                    )
                ...
                # 创建统计信息
                doc_stats = collect_node_stats(
                    node_with_title, stats, url, user_token, is_root=False
                )
                if doc_stats:
...
async def process_wiki_children_async(
    space_id: str, parent_token: str, url: str, user_token: str
) -> List[Dict[str, Any]]:
    """异步处理Wiki的所有子节点（优化版本）
    ...
    try:
        logger.info(
            f"异步扫描Wiki节点 {parent_token} (space_id: {space_id}) 的子节点..."
        )
        child_nodes = await get_all_child_nodes_async(
            space_id, user_token, parent_token
        )
...
def collect_node_stats_optimized(
    node: Union[NodeInfo, Any],
    stats: Optional[Any],
    source_url: str,
    update_time: int,
...
async def process_docx_async(
    file_token: str, file_type: str, url: str, user_token: str
) -> Optional[Dict[str, Any]]:
    """异步处理docx文档
    ...
    try:
        ...
        # 等待两个任务完成
        stats, metas = await asyncio.gather(
            stats_task, meta_task, return_exceptions=True
        )
...
async def process_wiki_node_async(
    file_token: str, space_id: str, url: str, user_token: str
) -> Tuple[Optional[Dict[str, Any]], Optional[WikiInfo]]:
    """异步处理Wiki节点
    ...
    try:
        ...
        # 获取统计信息
        stats_task = loop.run_in_executor(
            None, get_file_stats, file_token, "wiki", user_token
        )
        ...
        # 创建NodeInfo对象，使用真实标题
        node_info = NodeInfo(
            title=title, node_token=file_token, obj_token=file_token, obj_type=file_type
        )
...
def save_doc_stats(
    all_doc_stats: List[Dict[str, Any]], filename: str = "doc_stats.json"
) -> None:
    """保存文档统计信息到文件
...
def validate_config() -> bool:
    ...
    if (
        not config.app_id
        or config.app_id == "YOUR_APP_ID"
        or not config.app_secret
        or config.app_secret == "YOUR_APP_SECRET"
    ):
...
def get_document_statistics(
    ...
    for url_item in urls:
        ...
        if file_type == "wiki":
            ...
            logger.warning(
                f"警告：文件类型 '{file_type}' (来自URL: {url_item}) 不是 'wiki' 或 'docx'，跳过处理。"
            )
...
async def get_document_statistics_async(
    ...
    for url_item, file_type, file_token, space_id_task in url_tasks:
        if file_type == "wiki":
            ...
            # 创建Wiki处理任务
            wiki_task = process_wiki_node_async(
                file_token, space_id, url_item, user_token
            )
    ...
    if wiki_tasks:
        ...
        wiki_results = await asyncio.gather(
            *[task for _, _, _, task in wiki_tasks], return_exceptions=True
        )
        ...
        for i, (url_item, file_token, space_id, _) in enumerate(wiki_tasks):
            ...
            # 异步处理子节点
            child_task = process_wiki_children_async(
                space_id, file_token, url_item, user_token
            )
    ...
    if docx_tasks:
        ...
        docx_results = await asyncio.gather(
            *[task for _, task in docx_tasks], return_exceptions=True
        )
